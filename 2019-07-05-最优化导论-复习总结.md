...
- 梯度
  - $argmin_{\alpha}~\alpha_k=f(x^{(0)}-\alpha\nabla f(x^{(0)}))$
  - $x^{(k+1)}$
- 牛顿法
...
> 可行解是满足约束条件的解，基本解对应基向量的非基变量为零，基解不一定为可行解，可行解也不一定为基解，既是可行解又是基本解的解是基本可行解，最优解是基本可行解中使[目标函数](https://www.baidu.com/s?wd=目标函数&tn=SE_PcZhidaonwhc_ngpagmjz&rsv_dl=gh_pc_zhidao)达到最优的解。
